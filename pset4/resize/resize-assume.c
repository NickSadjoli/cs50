/**
 * Resizes BMP according to f given by user. This script assumes that f is >= 1 and is of rounded numbers (i.e. integers).
 */
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0, scancounter=0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        for (int j = 0, counter=0; j < bi.biWidth; j++)
        {
            ...
            if (f>=1.0) //if float f is more than or equals 1 (resizing to be equal size or larger)
            {
        ...
        if (f>=1.0) //if float f is more than 1 (resizing to be equal size or larger)
            {
